...
import static src.engine.board.Board.Builder;
        ...
        while (i < boardTiles.length) {
            switch (boardTiles[i]) {
                ...
                case 'Q':
                    ...
                    break;
                case 'K':
                    builder.setPiece(new King(Alliance.WHITE, i, blackKingSideCastle, blackQueenSideCastle));
                    i++;
                    break;
                case 'P':
    ...
    private static String calculateBoardText(final Board board) {
        ...
        for (int i = 0; i < BoardUtils.NUM_TILES; i++) {
            final String tileText = board.getPiece(i) == null ? "-" :
                    board.getPiece(i).getAlliance().isWhite() ? board.getPiece(i).toString() :
